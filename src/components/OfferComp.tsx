...
const OfferComp = ({ item, offerID }: { item?: any, offerID?: number }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            Post("/api/Offers/AskQuestionOffer", {
                "userID": user?.id,
                "offerID": offerID,
                ...
                <View className='space-y-3'>
                    ...
                    <View className='px-[10px] pb-[20px] space-y-3'>
                        ...
                        <View >
                            ...
                            <View className='flex-row items-center justify-between '>
                                <CustomInputs type='checkbox' title='Ulaşım' value={item?.extraServices?.some((item: number) => item === 1)} />
                                <CustomInputs type='checkbox' title='Konaklama' value={item?.extraServices?.some((item: number) => item === 2)} />
                                <CustomInputs type='checkbox' title='Refakatçi' value={item?.extraServices?.some((item: number) => item === 3)} />
                            </View>
                        </View>
                        ...
                        <View className='flex-row items-center justify-between'>
                            ...
                            <View className='w-[40%] '>
                                <Text numberOfLines={1} className='text-customGray font-poppins text-xs font-bold'>{item?.doctorName}</Text>
                            ...
                            <LikeUnlikeComp item={item} readOnly isFavorite={item?.isDoctorFavorite} />
                        </View>
                        ...
                        <CustomButtons type='solid' label='Soru Sor' icon='question' style={{ width: 100, alignSelf: "center" }} onPress={() => setVisible(true)} />
                    </View>
