...
const FirmServices = ({ route }: props) => {
    ...
    return (
        <FirmWrapper>
            <HandleData data={subServiceDetails} loading={loading} title='Firmanın Hizmeti Bulunamadı'>

                <View className='border border-customLightGray rounded-lg p-2 mb-3 space-y-3' style={{ width: SIZES.width * 0.95 }}>
                    <View className='flex-row items-center space-x-3'>
                        <InstutituonIcon />
                        <Text className='font-poppins font-medium text-sm text-customGray'>{route.params.type == 4 ? "Ameliyatsız" : "Ameliyatlı"} Operasyonlarımız</Text>
                        <View className='border-b h-1 flex-1'></View>
                    </View>

                    <Text className='font-poppins text-xs font-normal text-customGray'>{route.params?.serviceName}</Text>

                    <FlatList
                        data={subServiceDetails}
                        renderItem={({ item }) => <CustomButtons
                    ...
                    <View className='h-[130px] '>
                        <Carousel
                            data={selectedSub?.images?.map((img: any) => ({ imgUrl: img.image, title: "" }))}
                            renderItem={({ item }: any) => (
                                <Image source={{ uri: item?.imgUrl }} className='w-full h-full' resizeMode='cover' />
                            )}
                            sliderWidth={SIZES.width * 0.90}
                            itemWidth={130}
                            loop={true}
                            autoplay={true}
                        />
                    ...
                    <RenderHTML contentWidth={SIZES.width} source={{ html: selectedSub?.description }} />
